/** Predefined gossip interface catalogs.
 *  Each catalog groups compatible interfaces an agent can expose.
 *  Extensible — add new catalogs without breaking existing subscriptions.
 */
...
export const ALL_INTERFACES = Object.values(GOSSIP_CATALOGS).flatMap(c => c.interfaces);

export function catalogOf(iface: string): string | undefined {
    return Object.entries(GOSSIP_CATALOGS).find(([, v]) => v.interfaces.includes(iface))?.[0];
}

export function interfacesCompatible(agentInterfaces: string[], requiredInterfaces: string[]): boolean {
    if (requiredInterfaces.length === 0) return true;
    return agentInterfaces.some(i => requiredInterfaces.includes(i));
}
