    ...
    fastify.post<{
        ...
        if (ENV.NODE_ENV === 'production') {
            ...
            if (allowedIPs.length > 0 || allowedDomains.length > 0) {
                ...
                const ipOk = allowedIPs.length === 0 || allowedIPs.some((ip) => clientIP.startsWith(ip));
                const domainOk = allowedDomains.length === 0 || allowedDomains.some((d) => origin.includes(d));
