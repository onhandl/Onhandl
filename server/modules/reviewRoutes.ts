...
export const reviewRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.post<{
        ...
        if (!purchase) {
            return reply.code(403).send({ error: 'Only verified buyers can leave a review' });
        }
        ...
        if (existing) {
            ...
            await Review.create({ agentId, reviewerId, rating, comment });
        }

        // Recalculate and persist the average rating on the agent
        ...
        await AgentDefinition.updateOne(
            { _id: agentId },
            { $set: { 'marketplace.stats.rating': Math.round(avg * 10) / 10, 'marketplace.stats.reviews': count } }
        );
