...
export const userSettingsRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get('/payment-methods', async (request, reply) => {
        ...
        return {
            stripe: { enabled: !!saved.stripe?.enabled, stripeAccountId: user?.stripeAccountId || '' },
            crypto: saved.crypto || [],
        };
    });
    ...
    fastify.get('/api-keys', async (request, reply) => {
        ...
        return {
            gemini:        mask(k.gemini),
            openai:        mask(k.openai),
            openaiBaseUrl: k.openaiBaseUrl || '',
            openaiModel:   k.openaiModel || '',
            ollamaBaseUrl: k.ollamaBaseUrl || '',
            ollamaModel:   k.ollamaModel || '',
            hasGemini:  !!k.gemini,
            hasOpenai:  !!k.openai,
            hasOllama:  !!(k.ollamaBaseUrl),
        };
    });
