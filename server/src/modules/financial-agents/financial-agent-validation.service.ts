...
const transferFundsActionSchema = z.object({
    ...
    config: z.object({
        ...
        chain: z.enum(SUPPORTED_NETWORKS),
        label: z.string().min(1).optional(),
    }),
});
...
const networkConfigDraftSchema = z.object({
    ...
    recipientPolicy: z.enum(RECIPIENT_POLICIES).optional().default('allowlist'),
...
function validateDraftCrossFields(parsed: DraftFinancialAgentInput) {
    ...
    const networkMap = new Map(parsed.agent.networkConfigs.map((cfg) => [cfg.network, cfg]));
    ...
    for (const policy of parsed.policies) {
        ...
        for (const action of policy.actions) {
            ...
            const assertAssetAllowed = (networkCfg: DraftFinancialAgentInput['agent']['networkConfigs'][number], asset: string) => {
            ...
            if (action.type === 'TRANSFER_FUNDS' || action.type === 'SWAP_FUNDS' || action.type === 'INVEST_FUNDS') {
            ...
            if (action.type === 'ALLOCATE_FUNDS') {
                for (const allocation of action.config.allocations) {
                    if (!allocation.chain) {
                        throw {
                            code: 400,
                            message: 'ALLOCATE_FUNDS allocations must include chain explicitly',
                        };
                    }
