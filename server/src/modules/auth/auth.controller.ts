...
import {
    cookieAuthSecurity, idParamSchema,
    standardErrorResponses, userSchema,
} from '../../shared/docs';
...
function setAuthCookie(fastify: any, reply: any, userId: string, username: string, isAdmin = false) {
    ...
    const cookieOptions: Record<string, unknown> = {
        path: '/', httpOnly: true, secure: isProd, sameSite: 'lax', maxAge: 60 * 60 * 24 * 7,
    };
...
export async function authController(fastify: FastifyInstance) {
    ...
    fastify.post<{ Body: { email: string; code: string } }>(
        ...
        async (request, reply) => {
            ...
            try {
                ...
                return reply.code(201).send({ success: true, user: { id: user._id, username: user.username, email: user.email } });
            } catch (e: any) { return reply.code(e.code || 500).send({ error: e.message }); }
        }
    );
    ...
    fastify.post<{ Body: { username: string; password?: string } }>(
        ...
        schema: {
            ...
            response: {
                200: {
                    ...
                    type: 'object',
                    properties: { success: { type: 'boolean' }, user: userSchema },
                },
        ...
        async (request, reply) => {
            ...
            try {
                ...
                setAuthCookie(fastify, reply, String(user._id), user.username ?? '', user.isAdmin);
                return reply.send({ success: true, user: { id: user._id, username: user.username, email: user.email } });
            } catch (e: any) { return reply.code(e.code || 500).send({ error: e.message }); }
        }
    );
    ...
    fastify.post('/logout', {
        ...
        return reply.setCookie('auth_token', '', {
            path: '/', httpOnly: true, sameSite: 'lax',
            secure: process.env.NODE_ENV === 'production', expires: new Date(0),
    ...
    fastify.post<{ Body: { email: string } }>('/forgot-password', {
        schema: {
            ...
            response: {
                200: { description: 'Reset code sent (silently ignored if not registered)', type: 'object', properties: { message: { type: 'string' } } },
    ...
    fastify.post<{ Body: { email: string; code: string; newPassword: string } }>(
        ...
        schema: {
            ...
            response: {
                200: { description: 'Password updated', type: 'object', properties: { success: { type: 'boolean' }, message: { type: 'string' } } },
                ...standardErrorResponses([400, 500]),
            },
        },
    },
    ...
    fastify.get('/me/avatar', {
        ...
        schema: {
            ...
            security: [cookieAuthSecurity],
            response: {
                200: { description: 'Avatar URL', type: 'object', properties: { avatarUrl: { type: 'string' } } },
                ...standardErrorResponses([401, 404]),
            },
        },
    }, async (request, reply) => {
